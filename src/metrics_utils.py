...
def get_rgb_loss(rgb_pred, rgb_gt):
    # rgb_pred --> (BATCH_SIZE, NUM_POINTS, 3)
    # rgb_gt --> (BATCH_SIZE, NUM_POINTS, 3)
...
def get_iou(pcl_gt, pcl_pred, labels_gt, labels_pred, n_cls, metric, freq_wt=False):
    ...
    idx, _ = tf.meshgrid(range(batch_size), range(num_points),
                    indexing='ij')
    labels_pred_match = tf.gather_nd(labels_pred, tf.stack([idx,
                                    pts_match_fwd], -1))
    labels_gt_match = tf.gather_nd(labels_gt, tf.stack([idx,
                                    pts_match_bwd], -1))
...
def save_screenshots(_gt_scaled, _pr_scaled, img, screenshot_dir, fid, eval_set, ballradius, FLAGS, partseg=False, gt_labels=None, pr_labels=None, seg=False):
    import show3d_balls
    if not seg:
    ...
    if FLAGS.save_screenshots:
        ...
        for xangle, yangle in zip(xangles, yangles):
                gt_rot = show3d_balls.get2D(np_rotate(_gt_scaled, xangle=xangle, yangle=yangle), partseg=partseg, labels=gt_labels, ballradius=ballradius)
                result_rot = show3d_balls.get2D(np_rotate(_pr_scaled, xangle=xangle, yangle=yangle), partseg=partseg, labels=pr_labels, ballradius=ballradius)
                gts.append(gt_rot)
    ...
    if FLAGS.save_gifs:
        ...
        final = []
        for xangle, yangle in zip(xangles, yangles):
                gt_rot = cv2.cvtColor(show3d_balls.get2D(np_rotate(_gt_scaled, xangle=xangle, yangle=yangle), partseg=partseg, labels=gt_labels, ballradius=ballradius, background=(255,255,255), showsz=400), cv2.COLOR_BGR2RGB)
                result_rot = cv2.cvtColor(show3d_balls.get2D(np_rotate(_pr_scaled, xangle=xangle, yangle=yangle), partseg=partseg, labels=gt_labels, ballradius=ballradius, background=(255,255,255), showsz=400), cv2.COLOR_BGR2RGB)
